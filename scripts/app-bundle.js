...
define('app',['exports', 'aurelia-router', 'aurelia-cookie'], function (exports, _aureliaRouter, _aureliaCookie) {
  ...
  var App = exports.App = function () {
    ...
    App.prototype.insertTerm = function insertTerm() {
      ...
      var link = '<a href="/?context=' + this.insertTermObj.value + '" class="link">' + label + '</a>';
      var editor = document.querySelector('.bodyEditor');
      var html = editor.innerHTML;
    ...
    App.prototype.deleteClassItems = function deleteClassItems() {
      ...
      if (confirm(message)) {
        ...
        window.fetch(path).then(function (response) {
          ...
          _this26.cache = "refresh";
          _this26.fetchContext("class:_Class", 'list');
        }).catch(function (e) {
          return console.log(e);
        });
      }
    ...
    App.prototype.refreshBlocks = function refreshBlocks() {
      ...
      this.blocks.forEach(function (block) {
        ...
        console.log(block);
        if (block.type === "sp") {
          if (_this27.g[_this27.context].hasOwnProperty(block.predicate)) {
            var count = _this27.g[_this27.context][block.predicate].length;
            var index = -1;
            var contexts = _this27.g[_this27.context][block.predicate].map(function (object) {
              return object.value;
            });
            contexts.forEach(function (context) {
              window.fetch(_this27.server + '/lib/server.sjs?context=' + context).then(function (response) {
                return response.json();
              }).then(function (json) {
        ...
        if (block.type === "self") {
          ...
          if (_this27.g[_this27.context].hasOwnProperty(block.predicate)) {
            console.log(_this27.g[_this27.context][block.predicate]);
            var _count = _this27.g[_this27.context][block.predicate].length;
            var _index = -1;
            var graph = _this27.g;
            var _contexts = _this27.g[_this27.context][block.predicate].map(function (object) {
              return object.value;
            });
            block.html += _contexts.map(function (context) {
              return !_this27.isNullLink(context) ? block.template(context, graph, _index, _count) : '';
    ...
    App.prototype.getValidLink = function getValidLink(context, property) {
      ...
      if (this.g[context].hasOwnProperty(property)) {
        var validLink = this.g[context][property].find(function (value) {
          return !_this28.isNullLink(value);
        });
        return validLink;
      } else {
        return false;
