...
define('app',['exports', 'aurelia-router', 'aurelia-cookie'], function (exports, _aureliaRouter, _aureliaCookie) {
  ...
  var SplashScreen = function () {
    function SplashScreen(elemPath, startClass, endClass, interval) {
    ...
    SplashScreen.prototype.elements = function elements() {
      var _elements = Array.from(document.querySelectorAll(this.elemPath));
      console.log(_elements);
      return _elements;
    };

    SplashScreen.prototype.addClass = function addClass(className) {
    ...
    SplashScreen.prototype.init = function init() {
  ...
  var App = exports.App = function () {
    function App() {
      ...
      this.sortModeStates = [{ label: "📆⬇", value: "sortMode:_CreatedDateDesc", title: "Creation Date Descending" }, { label: "📆⬆", value: "sortMode:_CreatedDateAsc", title: "Creation Date Ascending" }, { label: "🛠⬇", value: "sortMode:_ModifiedDateDesc", title: "Last Modified Date Descending" }, { label: "🛠⬆", value: "sortMode:_ModifiedDateAsc", title: "Last Modified Date Asscending" }, { label: "A➡Z⬆", value: "sortMode:_AlphaAsc", title: "Alphanumeric Ascending" }, { label: "Z➡A⬇", value: "sortMode:_AlphaDesc", title: "Alphanumeric Descending" }, { label: "📄⬆", value: "sortMode:_PageAsc", title: "Page Ascending" }, { label: "📄⬇", value: "sortMode:_OrdinalDesc", title: "Page Descending" }];
      this.namespace = '';
    ...
    App.prototype.fetchContext = function fetchContext(context) {
      ...
      var dataOptions = { qRefine: '', noPush: false };
      if (this.cardEdit) {
      ...
      window.fetch(this.server + '/lib/server.sjs?context=' + context + '&cache=' + this.cache + '&uid=' + this.loginData.uid, options).then(function (blob) {
        ...
        if (!options.noPush) {
          _this3.history.unshift({ context: context, label: _this3.g[context]['term:prefLabel'][0].value.substr(0, 40) });
        }
        _this3.context = _this3.report['report:hasContext'][0].value;
        window.history.pushState({ context: _this3.context }, _this3.g[_this3.context]['term:prefLabel'][0].value, '/?context=' + _this3.context);
        ...
        _this3.getPreferredClasses(_this3.context);
        ...
        setTimeout(function () {
          ...
          _this3.addLinkContextMenu();
        }, 500);
        _this3.splashScreen.init();
      }).catch(function (err) {
        console.log("Error");
    ...
    App.prototype.addLinkContextMenu = function addLinkContextMenu() {
      if (Array.from(document.querySelectorAll('.linkContainer.au-target')).length === 0) {
        var contextMenu = CtxMenu('.linkContainer');

        contextMenu.addItem("Go", function (elt) {
          return app.fetchContext(elt.dataset.context);
        });
        contextMenu.addItem("Properties", function (elt) {
          return app.fetchContext(elt.dataset.context, 'properties');
        });
        contextMenu.addSeperator();
        contextMenu.addItem("Edit Label", function (elt) {
        ...
        contextMenu.addItem("Delete", function (elt) {
          return app.deleteCard(elt.dataset.context, elt.dataset.type);
        });
        contextMenu.addSeperator();
        contextMenu.addItem("Card in New Pane", function (elt) {
          return app.launchPage(elt.dataset.context, "card");
        });
        contextMenu.addItem("Properties in New Pane", function (elt) {
          return app.launchPage(elt.dataset.context, "properties");
        });
        contextMenu.addSeperator();
        contextMenu.addItem("Set to Draft", function (elt) {
          return app.setSingletonProperty(elt.dataset.context, 'term:hasPublicationStatus', 'publicationStatus:_Draft');
        });
        contextMenu.addItem("Set to Published", function (elt) {
          return app.setSingletonProperty(elt.dataset.context, 'term:hasPublicationStatus', 'publicationStatus:_Published');
        });
        console.log(contextMenu);
      }
    };

    App.prototype.setSingletonProperty = function setSingletonProperty(context, property, propertyValue) {
      var _this4 = this;

      var datatype = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : null;

      var options = { method: "post", body: JSON.stringify({ subject: context, predicate: property, object: propertyValue, datatype: datatype }, null, 4) };
      var path = this.server + '/lib/updateStatement.sjs';
      window.fetch(path, options).then(function (resp) {
        return resp.json();
      }).then(function (json) {
        console.log(json);app.fetchContext(_this4.context);
      }).catch(function (e) {
    ...
    App.prototype.inputSearch = function inputSearch() {
      ...
      if (this.q != "") {
        ...
        window.fetch(this.server + '/lib/search.sjs?q=' + this.q, options).then(function (blob) {
          ...
          _this6.searchData = json;
          console.log(_this6.searchData);
        }).catch(function (err) {
          return console.log(err);
        });
      }
    ...
    App.prototype.activateTab = function activateTab(predicate) {
      ...
      var sortMode = this.g[this.context].hasOwnProperty('class:isClassWithPreferredSortMode') && this.g[this.context]['class:isClassWithPreferredSortMode'] != null ? this.g[this.context]['class:isClassWithPreferredSortMode'][0].value : this.sortMode;
      ...
      window.fetch(path).then(function (response) {
        ...
        _this7.activeLinksData = json;
        _this7.linksContent = json.content.content || "";

        _this7.page = json.page;
        if (resetPage) {
          _this7.page = 1;
        }
        _this7.totalPages = json.numPages;
      });
    };
    ...
    App.prototype.sort = function sort() {
      ...
      this.links.forEach(function (link) {
        _this8.g[link].link = link;
      });
      var linkNodes = this.links.map(function (link) {
        return _this8.g[link];
      });
    ...
    App.prototype.displayLiteral = function displayLiteral(item) {
      ...
      switch (datatype) {
        ...
        case "xsd:date":
          {
            return '' + new Date(item.value).toLocaleString(["en-us"], { timeZone: 'UTC' }).split(/\, /)[0];
          };
        ...
        case "xsd:anyURI":
          {
            ...
            var protocol = item.value.match(/^http/) ? '' : 'https://';
            return '<a href="' + protocol + item.value + '" target="_blank" class="link">' + item.value.replace(/(http.+?\/\/)(.+?\/).*/, '$1$2...') + '</a>';
          };
        case "xsd:email":
          return '<a href="mailto:' + item.value + '" target="_blank" class="link">' + item.value + '</a>';
        case "xsd:telephone":
          return '<a href="tel:' + item.value.replace(/\-|\(|\)|\./g, '') + '" target="_blank" class="link">' + item.value + '</a>';
        case "xsd:anyURL":
    ...
    App.prototype.saveCard = function saveCard() {
      ...
      window.fetch(path, { body: JSON.stringify(this.activeCard), method: "POST" }).then(function (response) {
        ...
        _this11.cache = "refresh";
        _this11.fetchContext(_this11.activeCard.curie, "card");
      }).catch(function (e) {
        return console.log(e);
      });
      this.cardEdit = false;
    ...
    App.prototype.filterBody = function filterBody(content) {
      ...
      var filters = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : ['eval', 'table'];
      ...
      filters.forEach(function (filter) {
        switch (filter) {
          case "box":
            input = _this12.boxFilter(input);return input;break;
          case "lists":
            input = _this12.listsFilter(input);return input;break;
          case "eval":
            input = _this12.evalFilter(input);return input;break;
          case "table":
            input = _this12.tableFilter(input);return input;break;
          default:
            input;
        }
      });
    ...
    App.prototype.tableFilter = function tableFilter(input) {
      var _this13 = this;

      var context = this.context;
      var ct = 0;
      var parser = new DOMParser();
      input = input.replace(/(\[macro\])(.*?)(\[\/macro\])/gi, function (match, p1, p2, p3) {
        return '' + p1 + p2.replace(/\<\/?div\>/gi, '').replace("&#160;", " ", "g") + p3;
      });

      input = input.replace(/\[macro\]\s*?\[query\](.+?)\[\/query\]\s*?\[template\](.+?)\[\/template\]\s*?\[style\](.+?)\[\/style\]\s*?\[\/macro\]/gi, function (match, query, template, style) {
        ...
        var htmlTemplate = eval(template);
        var options = { method: "POST", body: JSON.stringify({ query: query }) };
        var path = _this13.server + '/lib/clientEval.sjs';
        var tableId = context.replace(/\:/, '_') + '_table_' + ct;
        ct += 1;
        fetch(path, options).then(function (resp) {
          return resp.json();
        }).then(function (json) {
          var rows = json;
          var keys = Object.keys(rows[0]);
          var html = '<style type="css">' + style + '</style> ' + htmlTemplate(rows, keys);
          console.log(html);
    ...
    App.prototype.evalFilter = function evalFilter(input) {
      var context = this.context;
      input = input.replace(/\[eval\:(.*?)\]/g, function (match, $1) {
        return eval($1);
      });
      return input;
    };
    ...
    App.prototype.editCardProperties = function editCardProperties() {
      ...
      Object.keys(this.g[this.context]).forEach(function (predicate) {
        if (!Array.isArray(_this14.g[_this14.context][predicate])) {
          _this14.g[_this14.context][predicate] = [];
        };
        _this14.g[_this14.context][predicate].forEach(function (object) {
          if (object.type === "uri") {
            console.log(object.value, _this14.g.hasOwnProperty(object.value));
            if (_this14.g.hasOwnProperty(object.value)) {
              if (_this14.g[object.value] != null) {
                object.label = _this14.g[object.value]['term:prefLabel'][0].value;
    ...
    App.prototype.saveCardProperties = function saveCardProperties() {
      ...
      var update = Array.from(Object.keys(this.g[this.context])).forEach(function (predicate) {
        var objects = _this15.g[_this15.context][predicate] || [];
      ...
      console.log(output);
      var newRecord = { "@context": this.namespace, "graph": this.g, subject: this.context, curie: curie };
      ...
      window.fetch(path, { method: "POST", body: JSON.stringify(newRecord, null, 4) }).then(function (response) {
        ...
        _this15.fetchContext(curie, _this15.mode || "card");
      }).catch(function (e) {
        return console.log(e);
      });
    };
    ...
    App.prototype.saveDuplicateCardProperties = function saveDuplicateCardProperties() {
      ...
      var buffer = [];
      var update = Array.from(Object.keys(this.g[this.context])).forEach(function (predicate) {
        var objects = Array.from(_this16.g[_this16.context][predicate]).filter(function (objects) {
          return objects != null;
        });
        Array.from(_this16.g[_this16.context][predicate]).forEach(function (object) {
          if (object.type === 'uri') {
            buffer.push(curie + ' ' + predicate + ' ' + object.value + '.');
          } else {
            buffer.push(curie + ' ' + predicate + ' """' + object.value + '"""^^' + (object.datatype || 'xsd:string') + '.');
      ...
      var triples = buffer.join('\n');
      var prolog = Object.keys(this.ns).map(function (prefix) {
        return 'prefix ' + prefix + ': <' + _this16.ns[prefix] + '>';
      }).join('\n');
      var output = prolog + '\ndelete {' + context + ' ?p ?o}\ninsert {\n  ' + triples + '\n}\nwhere {\n  optional {\n  ' + context + ' ?p ?o\n  }\n}';
      console.log(output);
    ...
    App.prototype.logout = function logout() {
      ...
      window.fetch(path, params).then(function (response) {
        ...
        console.log(json);
        _this18.loginData = json;
        _this18.userRole = new Set(_this18.loginData.permissions);
        _aureliaCookie.AureliaCookie.delete("login");
        _this18.cache = "cached";
        _this18.fetchContext(_this18.context);
        _this18.validateUserContext();
      }).catch(function (e) {
        return console.log(e);
      });
    };
    ...
    App.prototype.processLogin = function processLogin() {
      ...
      window.fetch(path, params).then(function (response) {
        ...
        console.log(json);
        _this19.loginData = json;
        _this19.userRole = new Set(_this19.loginData.permissions);
        if (!_this19.loginData.status) {
          alert("Log-in failed.");
        } else {
          _aureliaCookie.AureliaCookie.set("login", JSON.stringify(_this19.loginData), { expiry: 60, path: '', domain: '', secure: false });
        }
      }).catch(function (e) {
        return console.log(e);
      });
    ...
    App.prototype.newCard = function newCard(ctx) {
      var label = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : this.newModalLabel;
      var asProperty = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;

      var _this20 = this;

      var preferredProperty = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : '';
      ...
      window.fetch(path).then(function (response) {
        ...
        _this20.templateProperties = json.templateData;
        console.log(_this20.templateProperties);
        _this20.generateEntityId(_this20.context);
        Array.from(window.document.querySelectorAll('.modal-body')).forEach(function (elem) {
          return elem.scrollTop = 0;
        });
        _this20.newModal.open();
      }).catch(function (e) {
        return console.log(e);
      });
      this.newModal.open();
    ...
    App.prototype.refreshTemplateProperties = function refreshTemplateProperties() {
      var _this21 = this;
      ...
      window.fetch(path).then(function (response) {
        ...
        _this21.predicates = json.templateData;
        console.log(_this21.predicates);
      }).catch(function (e) {
        return console.log(e);
      });
    };
    ...
    App.prototype.deleteCard = function deleteCard() {
      ...
      if (confirm('Are you sure you wish to delete this item?')) {
        ...
        window.fetch(path).then(function (response) {
          ...
          _this22.refreshApp();
          _this22.fetchContext(landing, "card");
        }).catch(function (err) {
          return console.log(err);
        });
      }
    ...
    App.prototype.processNewCard = function processNewCard() {
      ...
      var prolog = Object.keys(this.ns).map(function (prefix) {
        return 'prefix ' + prefix + ': <' + _this23.ns[prefix] + '>';
      }).join('\n');
      console.log("ToDo: Complete the update of the new entry.");
      ...
      this.templateProperties.filter(function (predicate) {
        ...
        return _this23.activeCard.predicateEntries.push(Object.assign(predicate, {}));
      });
      console.log("Predicate Entries", this.activeCard.predicateEntries);
      this.activeCard.predicateEntries.filter(function (property) {
        return property.curie === 'term:hasPublicationStatus' && (!property.value || property.value === '');
      }).forEach(function (property) {
        return property.value = 'publicationStatus:_Draft';
      });
      ...
      window.fetch(path, { method: "POST", body: JSON.stringify(this.activeCard, null, 4) }).then(function (response) {
        ...
        console.log(_this23.activeCard);

        _this23.fetchContext(_this23.activeCard.curie, 'card');
      }).catch(function (e) {
        return console.log(e);
      });
    };
    ...
    App.prototype.processDuplicateCard = function processDuplicateCard() {
      ...
      var prolog = Object.keys(this.ns).map(function (prefix) {
        return 'prefix ' + prefix + ': <' + _this24.ns[prefix] + '>';
      }).join('\n');
      ...
      Object.keys(this.g[this.context]).forEach(function (predicate) {
        if (!ignorePredicates.has(predicate)) {
          _this24.g[_this24.context][predicate].forEach(function (object) {
            if (object.type === 'literal') {
              triple = _this24.activeCard.curie + ' ' + predicate + ' ' + object.value + '.';
            } else {
              triple = _this24.activeCard.curie + ' ' + predicate + ' """' + object.value + '"""^^' + (object.datatype || 'xsd:string') + '.';
            }
            triples.push(triple);
          });
        }
    ...
    App.prototype.getTerms = function getTerms(property) {
      ...
      window.fetch(path).then(function (response) {
        ...
        _this25.linkPropertyList = {};
        _this25.linkPropertyList[property] = json.results;
      });
      return [];
    };
    ...
    App.prototype.updateInstanceList = function updateInstanceList(classContext) {
      ...
      window.fetch(path).then(function (response) {
        ...
        _this26.instanceList[classContext] = {};
        _this26.instanceList[classContext][predicate] = json;
      });
    };
    ...
    App.prototype.filterComplianceTest = function filterComplianceTest() {
      ...
      window.fetch(path, { method: "POST", body: JSON.stringify(constraintObj, null, 4) }).then(function (response) {
        ...
        _this27.instanceList[classContext] = {};
        _this27.instanceList[classContext][predicate] = json;
      });
    };
    ...
    App.prototype.addProperty = function addProperty(property) {
      ...
      var contextType = this.g[context]['rdf:type'][0].value;
      var path = this.server + '/lib/properties.sjs?type=' + contextType + '&cache=refresh';
      console.log(path);
      fetch(path).then(function (response) {
        return response.json();
      }).then(function (json) {
        _this28.availableProperties = json.results;
        console.log(_this28.availableProperties);
        _this28.selectedProperty = property;
        if (property != null) {
          _this28.setPropertyListValue(property);
        };
        _this28.addPropertyModal.open();
      }).catch(function (e) {
        return console.log(e);
      });
    };
    ...
    App.prototype.setPropertyListValue = function setPropertyListValue(predicate) {
      ...
      if (this.activeProperty.nodeKind === 'nodeKind:_IRI') {
        ...
        fetch(path).then(function (response) {
          ...
          _this29.activePropertyValues = json;
        }).catch(function (e) {
          return console.log(e);
        });
      }
    ...
    App.prototype.newConstraint = function newConstraint() {
      ...
      fetch(path).then(function (response) {
        ...
        _this30.availableProperties = json.results;
        _this30.activeConstraint.predicate = "";
        _this30.activeConstraint.object = "";
        _this30.addConstraintModal.open();
      }).catch(function (e) {
        return console.log(e);
      });
    };
    ...
    App.prototype.updateInsertTerm = function updateInsertTerm() {
      ...
      var searchItem = this.searchData.results.find(function (searchItem) {
        return searchItem.s === _this31.insertTermObj.value;
      });
    ...
    App.prototype.updateInsertImage = function updateInsertImage() {
      ...
      var searchItem = this.searchData.results.find(function (searchItem) {
        return searchItem.s === _this32.insertTermObj.value;
      });
    ...
    App.prototype.createTermDlg = function createTermDlg() {
    ...
    App.prototype.createTerm = function createTerm() {
      var _this33 = this;

      var ctObj = this.createTermObj;
      var cleanTitle = ctObj.title;
      var className = ctObj.type.replace(/.*?\:_(.+?)$/, "$1");
      var prefix = '' + className.substr(0, 1).toLowerCase() + className.substr(1);
      var id = this.tokenize(cleanTitle);
      if (id === "") {
        id = this.tokenize(new Date().toISOString());
      }
      ...
      console.log(ctObj);
      var options = { method: "POST", body: JSON.stringify(ctObj, null, 4) };
      window.fetch('http://gracie.semanticdatagroup.com/lib/newCard.sjs', options).then(function (resp) {
        return resp.json();
      }).then(function (json) {
        console.log(json);
        _this33.classify();
    ...
    App.prototype.cancelCreateTerm = function cancelCreateTerm() {};
    ...
    App.prototype.deleteClassItems = function deleteClassItems() {
      ...
      if (confirm(message)) {
        ...
        window.fetch(path).then(function (response) {
          ...
          _this34.cache = "refresh";
          _this34.fetchContext("class:_Class", 'card');
        }).catch(function (e) {
          return console.log(e);
        });
      }
    ...
    App.prototype.createImageFile = function createImageFile() {
      ...
      if (url.startsWith('data:image')) {
        ...
        window.fetch(this.server + '/lib/uploadImage.sjs', options).then(function (resp) {
          ...
          var path = _this35.server + '/lib/getImage.sjs?path=' + json.filepath;
          target.image = path;
        }).catch(function (e) {
          return console.log(e);
        });
    ...
    App.prototype.createFile = function createFile() {
      ...
      if (url.startsWith('data:')) {
        ...
        window.fetch(this.server + '/lib/uploadFile.sjs', options).then(function (resp) {
          ...
          var path = _this36.server + '/lib/getFile.sjs?path=' + json.filepath;
    ...
    App.prototype.refreshBlocks = function refreshBlocks() {
      ...
      this.blocks.forEach(function (block) {
        ...
        if (new Set(block.mode).has(_this37.mode)) {
          var container = document.querySelector('.' + block.container);
          if (container != null) {
            ...
            console.log(block);
            if (block.type === "sp") {
              if (_this37.g[_this37.context].hasOwnProperty(block.predicate)) {
                var count = _this37.g[_this37.context][block.predicate].length;
                var index = -1;
                var contexts = _this37.g[_this37.context][block.predicate].map(function (object) {
                  return object.value;
                });
                contexts.forEach(function (context) {
                  window.fetch(_this37.server + '/lib/server.sjs?context=' + context).then(function (response) {
                    return response.json();
                  }).then(function (json) {
            ...
            if (block.type === "self") {
              ...
              if (_this37.g[_this37.context].hasOwnProperty(block.predicate)) {
                console.log(_this37.g[_this37.context][block.predicate]);
                var _count = _this37.g[_this37.context][block.predicate].length;
                var _index = -1;
                var graph = _this37.g;
                var _contexts = _this37.g[_this37.context][block.predicate].map(function (object) {
                  return object.value;
                });
                block.html += _contexts.map(function (context) {
                  return !_this37.isNullLink(context) ? block.template(context, graph, _index, _count) : '';
    ...
    App.prototype.getValidLink = function getValidLink(context, property) {
      ...
      if (this.g[context].hasOwnProperty(property)) {
        var validLink = this.g[context][property].find(function (value) {
          return !_this38.isNullLink(value);
        });
        return validLink;
      } else {
        return false;
    ...
    App.prototype.updateConfiguration = function updateConfiguration() {
      ...
      var path = this.server + '/lib/configure.sjs?host=' + this.serverDomain;
      window.fetch(path).then(function (response) {
        return response.json();
      }).then(function (json) {
        _this39.config = json;
        _this39.config.mode = _this39.params.mode || _this39.config.mode;
        _this39.context = _this39.params != null ? _this39.params.context || _this39.config.defaultPage : _this39.config.defaultPage;

        _this39.fetchContext(_this39.context, _this39.mode);
      }).catch(function (e) {
        return console.log(e);
      });
    };

    App.prototype.updateConfigurationOld = function updateConfigurationOld() {
      var _this40 = this;

      var path = this.server + '/lib/configure.sjs?host=' + this.hostDomain;
      window.fetch(path).then(function (response) {
        return response.json();
      }).then(function (json) {
        var graph = json.graph;
        if (graph.hasOwnProperty('class:_Configuration')) {
          if (graph["class:_Configuration"].hasOwnProperty('class:hasActiveConfiguration')) {
            ...
            if (configuration != _this40.oldConfiguration) {
              _this40.oldConfiguration = configuration;
              var _path2 = _this40.server + '/lib/server.sjs?context=' + configuration;
              window.fetch(_path2).then(function (response) {
                return response.json();
              }).then(function (json) {
                var graph = json.graph;
                _this40.theme = graph[configuration].hasOwnProperty('configuration:hasTheme') ? graph[configuration]['configuration:hasTheme'][0].value : "";
                _this40.defaultLabel = graph[configuration].hasOwnProperty('configuration:hasSiteName') ? graph[configuration]['configuration:hasSiteName'][0].value : "";
                _this40.defaultIcon = graph[configuration].hasOwnProperty('configuration:hasIcon') ? graph[configuration]['configuration:hasIcon'][0].value : "";
                _this40.defaultPage = graph[configuration].hasOwnProperty('configuration:hasHomePage') ? graph[configuration]['configuration:hasHomePage'][0].value : "page:_Home";
                _this40.activeCSS = graph[configuration].hasOwnProperty('configuration:hasCSS') ? graph[configuration]['configuration:hasCSS'][0].value : "";
                _this40.footer = graph[configuration].hasOwnProperty('configuration:hasFooter') ? graph[configuration]['configuration:hasFooter'][0].value : "";
                _this40.topBannerMessage = graph[configuration].hasOwnProperty('configuration:hasTopBanner') ? graph[configuration]['configuration:hasTopBanner'][0].value : "";
                var mode = graph[configuration].hasOwnProperty('configuration:hasMode') ? graph[configuration]['configuration:hasMode'][0].value : "card";
                _this40.mode = _this40.params.mode || mode;
                _this40.context = _this40.params != null ? _this40.params.context || _this40.defaultPage : _this40.defaultPage;

                _this40.fetchContext(_this40.context, _this40.mode);
              }).catch(function (e) {
                return console.log(e);
              });
            }
    ...
    App.prototype.getPreferredClasses = function getPreferredClasses() {
      var _this41 = this;
      ...
      var preferredProperties = this.g[contextClass].hasOwnProperty('class:hasPreferredProperty') ? this.g[contextClass]['class:hasPreferredProperty'] : [];
      console.log(preferredProperties);
      var childPrefixes = preferredProperties.map(function (property) {
        return property.value.split(':')[0];
      });
      console.log(childPrefixes);
      var childClasses = childPrefixes.map(function (childPrefix) {
        return _this41.instanceList['class:_Class']['rdf:type'].filter(function (childClass) {
          return childClass.prefix === childPrefix;
        });
      });
    ...
    App.prototype.loadMenu = function loadMenu(context, selector) {
      ...
      window.fetch(path).then(function (response) {
        ...
        var menu = json.data;
        var html = _this42.traverseMenu(menu, context);
        ...
        case "userRole:_DataSteward":
          {
            ...
            this.userValidated = this.loginData.data.hasOwnProperty('user:hasEditClass') ? this.loginData.data['user:hasEditClass'].find(function (editClass) {
              return editClass.value === typeClass || editClass.value === _this44.context;
            }) != null : false;
          };break;
        default:
          this.userValidated = false;
    ...
    App.prototype.classify = function classify() {
      var _this45 = this;

      var message = { context: this.context, description: this.activeCard.body, uid: this.loginData.context };
      console.log(message);
      var path = this.server + '/lib/enrich.sjs';
      var options = { "method": "POST", body: JSON.stringify(message, null, 4) };
      window.fetch(path, options).then(function (resp) {
        return resp.json();
      }).then(function (json) {
        console.log(json);
        _this45.activeCard.body = json.description;
        var topics = json.topics;
        console.log(topics);
        var context = _this45.context;
        topics.forEach(function (topic) {
          var predicate = topic.predicate;
          var curie = topic.curie;
          var doesPredicateExist = _this45.g[_this45.context].hasOwnProperty(predicate);
          var obj = { type: "uri", value: curie };
          if (predicate != null) {
            if (Array.isArray(_this45.g[context][predicate])) {
              _this45.g[context][predicate].push(obj);
            } else {
              _this45.g[context][predicate] = [obj];
            }
          }
        });
      }).catch(function (e) {
    ...
    App.prototype.getPage = function getPage() {
      var _this46 = this;

      var url = prompt("Enter page URL");
      var path = '/lib/getPage.sjs?url=' + url;
      fetch(path).then(function (resp) {
        ...
        var meta = {};
        Array.from(dom.querySelectorAll('meta[property]')).map(function (metaElt) {
          var property = metaElt.getAttribute('property');
          var content = metaElt.getAttribute('content');
        ...
        if (url.match(/instagram\.com/)) {
          _this46.activeCard.title = meta['og:title'].replace(/<.+?>/g, ' ').replace(/\s+/g, ' ');
          _this46.activeCard.curie = 'webPost:_' + meta['og:url'].replace(/[^A-Za-z0-9]/g, '').replace(/\:/g, '');
          _this46.activeCard.type = 'class:_WebPost';
          _this46.activeCard.image = meta['og:image'];
          _this46.activeCard.body = meta['og:description'];
          _this46.activeCard.externalURL = meta['og:url'];
          if (meta.hasOwnProperty('instapp:hashtags')) {
            _this46.activeCard.body += '<br/>' + meta['instapp:hashtags'].map(function (hashtag) {
              return '#' + hashtag;
            }).join(' ');
          }
          _this46.activeCard.body += '<br/><br/><b>Social Media Site:</b> Instagram<br/>';
        }
        if (url.match(/pinterest\.com/)) {
          ...
          _this46.activeCard.body += '<br/><br/><b>Social Media Site:</b> Pinterest<br/>';
        } else {
          var xp = new XPath(dom);
          _this46.activeCard.title = xp.getNodes("//title")[0].textContent.trim();
          _this46.activeCard.body = Array.from(dom.querySelectorAll('BODY>*:not(SCRIPT)')).map(function (node) {
            return node.outerHTML;
          }).join('\n');

          _this46.activeCard.image = meta['og:image'];
          _this46.activeCard.externalURL = url;
          _this46.generateEntityId(_this46.context);
        }
        console.log(meta);
      }).catch(function (e) {
        return console.log(e);
    ...
    App.prototype.getFileAsync = function () {
      var _ref = _asyncToGenerator(regeneratorRuntime.mark(function _callee(url) {
        var path, resp, docText;
        return regeneratorRuntime.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                path = '/lib/getPage.sjs?url=' + url;
                _context.next = 3;
                return window.fetch(path, { method: "GET", mode: "cors" });
              ...
              case 6:
                docText = _context.sent;
                return _context.abrupt('return', docText);
    ...
    App.prototype.getWebPage = function () {
        ...
        return regeneratorRuntime.wrap(function _callee2$(_context2) {
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                url = prompt("Paste in URL of web page to load.");
              ...
              case 4:
                ...
                this.activeCard.body = xp.getNodes("//BODY/*").map(function (node) {
                  return node.outerHTML;
                }).join('\n');
