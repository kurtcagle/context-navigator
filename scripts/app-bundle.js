  ...
  var App = exports.App = function () {
    ...
    App.prototype.fetchContext = function fetchContext(context) {
      ...
      window.fetch(this.server + '/lib/server.sjs?context=' + context + '&cache=' + this.cache, options).then(function (blob) {
        ...
        console.log(err);_this3.wait = false;
      });
    };
    ...
    App.prototype.sort = function sort() {
      ...
      if (linkNodes.length > 1) {
        linkNodes.sort(function (a, b) {
          return a["term:prefLabel"][0].value <= b["term:prefLabel"][0].value ? -1 : 1;
        });
      }
    ...
    App.prototype.linkCount = function linkCount() {};
    ...
    App.prototype.processLogin = function processLogin() {
      ...
      window.fetch(path, params).then(function (response) {
        ...
        if (!_this15.loginData.status) {
          ...
          _aureliaCookie.AureliaCookie.set("login", JSON.stringify(_this15.loginData), { expiry: 1, path: '', domain: '', secure: false });
        }
      }).catch(function (e) {
        return console.log(e);
      });
      ...
      if (cardType === "class:_Class") {
        ...
        if (confirm('Are you sure you wish to delete this ' + typeLabel.toLowerCase() + '?')) {
          ...
          window.fetch(path).then(function (response) {
            ...
            console.log(text);
            location.href = _this16.client + '?context=' + cardType + '&cache=replace';
          }).catch(function (err) {
            return console.log(err);
          });
        }
    ...
    App.prototype.insertTerm = function insertTerm(searchItem) {
      console.log("Entering insertTerm()");
      this.insertTermModal.close();
      this.q = "";
      var link = '<a href="/?context=' + searchItem.s + '" class="link">' + searchItem.prefLabel + '</a>';
      var editor = document.querySelector('.bodyEditor');
      var html = editor.innerHTML;
    ...
    App.prototype.deleteClassItems = function deleteClassItems() {
      ...
      if (confirm('Warning! This will delete the class "' + this.g[this.context]['term:prefLabel'][0].value + '", all properties for that class and all items in the class. It can not be undone. Are you sure you want to continue?')) {
        ...
        var path = this.server + '/lib/deleteClassItems.sjs?context=' + this.context;
        window.fetch(path).then(function (response) {
          return response.text();
        }).then(function (text) {
          _this25.cache = "refresh";
          _this25.fetchContext("class:_Class", 'list');
        }).catch(function (e) {
          return console.log(e);
        });
      }
    ...
    App.prototype.loadBlocks = function loadBlocks() {
      this.blocks = [{
        ...
        template: function template(context, graph, index, count) {
          return '<span onclick="window.app.fetchContext(\'' + context + '\')" class="topic link" \n      >' + graph[context]['term:prefLabel'][0].value + '</span>';
        }
      }];
    };
    ...
    App.prototype.refreshBlocks = function refreshBlocks() {
      ...
      this.blocks.forEach(function (block) {
        ...
        console.log(block);
        if (block.type === "sp") {
          if (_this26.g[_this26.context].hasOwnProperty(block.predicate)) {
            var count = _this26.g[_this26.context][block.predicate].length;
            var index = -1;
            var contexts = _this26.g[_this26.context][block.predicate].map(function (object) {
              return object.value;
            });
            contexts.forEach(function (context) {
              window.fetch(_this26.server + '/lib/server.sjs?context=' + context).then(function (response) {
                return response.json();
              }).then(function (json) {
        ...
        if (block.type === "self") {
          ...
          if (_this26.g[_this26.context].hasOwnProperty(block.predicate)) {
            var _count = _this26.g[_this26.context][block.predicate].length;
            var _index = -1;
            var graph = _this26.g;
            var _contexts = _this26.g[_this26.context][block.predicate].map(function (object) {
              return object.value;
            });
            block.html += _contexts.map(function (context) {
              return block.template(context, graph, _index, _count);
